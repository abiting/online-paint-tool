...
2026-08-29 的公開瀏覽器錯誤顯示直接交由 ONNX Runtime 下載跨網域模型仍可能在重新導向期間失敗。載入流程調整為先用 `fetch` 取得模型 `ArrayBuffer`（使用瀏覽器 HTTP 快取、略過帳號憑證），再交給 WASM Runtime 建立 session；第一次失敗後延遲 700 ms 重試一次。兩次均失敗時保留原圖，使用者重新點擊去背即可再次嘗試。
...
去背完成後可進入「修補去背」。畫布呈現目前合成結果，保留筆從原始來源取回像素、移除筆清空像素；提交時系統比較修補前後的 alpha 與原始來源像素，將使用者真正保留／移除的區域回寫到 `backgroundMask`，再由原始圖片與更新後遮罩重新合成。此設計不會把修補直接烤進原圖，日後仍可調整邊緣柔化與邊緣清理，也可重新進入修補模式。
