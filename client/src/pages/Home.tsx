...
const localeCopy = {
  "zh-Hant": {
    ...
    faq: [
      ...
      ["AbiPaint 還有什麼功能？", "除了修改圖片尺寸，AbiPaint 也提供筆刷、圖形、文字、去背等多種功能，適合學生、教師、設計師、行銷人員等各行各業使用。"],
...
export default function Home() {
  ...
  const selectedShapeOrText = Boolean(selectedShape || selectedText);
  const canUseEraser = !selectedShapeOrText && (canUseImageEraser || hasErasableBrushStrokes);
  const eraserDisabledHint = selectedShapeOrText
    ? tr("請先取消圖形或文字選取", "Deselect the shape or text first")
    : tr("請先選取圖片或建立筆刷內容", "Select an image or create brush marks first");
                ...
                <section className="developer-projects" aria-labelledby="developer-projects-title">
                  ...
                  <div className="developer-work-list">
                    {developerWorks.map((work) => {
                      ...
                      const workHref = isEnglish && "hrefEnglish" in work ? work.hrefEnglish : work.href;
                      return (
                        <a key={work.href} className="developer-work" href={workHref} target="_blank" rel="noopener noreferrer">
        ...
        <section ref={workspaceRef} className="workspace" aria-label={copy.canvasWorkspace}>
          ...
          <footer className="workspace-footer product-footer" aria-label="AbiPaint 工具說明">
            ...
            <div className="product-footer-meta">
              ...
              <div className="product-build-meta" aria-label="AbiPaint version 1.0.5">
                <a className="product-version-label" href={isEnglish ? "/en/guide/" : "/guide/"} title={isEnglish ? "Read the AbiPaint guide" : "閱讀 AbiPaint 使用指南"}>v1.0.5</a>
                <a
